...
class NeuroChat {
    ...
    loadData() {
        const savedChats = storage.load('chats');
        const savedMessages = storage.load('messages');
        const savedProfile = storage.load('profile');
        
        if (savedChats) this.chats = savedChats;
        if (savedMessages) this.messages = savedMessages;
        if (savedProfile) this.userProfile = savedProfile;
    }

    saveData() {
        storage.save('chats', this.chats);
        storage.save('messages', this.messages);
        storage.save('profile', this.userProfile);
    }
